...
import org.withmystar.shell.shared.reflection.ReflectionUtils;
...
public class TermuxUtils {
     ...
     *
     * If your third-party app is targeting sdk `30` (android `11`), then it needs to add `org.withmystar.shell`
     * package to the `queries` element or request `QUERY_ALL_PACKAGES` permission in its
     * `AndroidManifest.xml`. Otherwise it will get `PackageSetting{...... org.withmystar.shell/......} BLOCKED`
     ...
     * <manifest
     *     <queries>
     *         <package android:name="org.withmystar.shell" />
     *    </queries>
     * </manifest>
     * }
     *
    ...
    public static String geAPTInfoMarkdownString(@NonNull final Context context) {
        ...
        InputStream inputStream = context.getResources().openRawResource(org.withmystar.shell.shared.R.raw.apt_info_script);
        try {
